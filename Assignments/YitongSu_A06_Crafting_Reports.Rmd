...
- The second column should have the header `Value` and include the metadata values: \newline `EPAair_O3_PM25_NC1819_Processed.csv`, `2018-2019`, and `EPA Air Quality System (AQS)`.
...
```{r boxplot1, echo=FALSE, fig.height=3, fig.align="left", fig.cap="2018 monthly Ozone levels"}
library(ggplot2)
...
P1 <- ggplot(Ozone.2018, aes(x=factor(Month, levels=1:12, labels = month.abb),
                       y=Ozone))+
  geom_boxplot()+
  labs(title = "2018 Monthly Ozone Concentrations",
       x = "Month", y = expression(Ozone ~ Concentration ~ (µg/m^3)))+
  scale_x_discrete(name = "Month", drop = FALSE)

P1
```

```{r boxplot2, echo=FALSE, fig.height=3, fig.align="left", fig.cap="2019 monthly Ozone levels"}

Ozone.2019 <- epa_data %>%
  drop_na(Ozone) %>%
  filter(Year==2019)

P2 <- ggplot(Ozone.2019, aes(x=factor(Month, levels=1:12, labels = month.abb),
                       y=Ozone))+
  geom_boxplot()+
  labs(title = "2019 Monthly Ozone Concentrations",
       x = "Month", y = expression(Ozone ~ Concentration ~ (µg/m^3)))+
  scale_x_discrete(name = "Month", drop = FALSE)
...
https://github.com/sytspg/EDE_Fall2025

I did not use AI in the completion of this assignment.
-Yitong Eric Su October 16, 2025
