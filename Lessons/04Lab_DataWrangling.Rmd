...
NTL.phys.data.1999.Tuesday <- NTL.phys.data %>%
  filter(
...
Let's say we want to take our raw dataset (NTL.phys.data), *then* filter the data for Peter and Paul lakes, *then* write temperature and observation information, and *then* add a column for temperature in Fahrenheit: 
...
NTL.phys.data.processed <- 
  ...
  write(lakename, sampledate:temperature_C) %>%
...
NTL.phys.data.PeterPaul.gathered <-
  pivot_longer(NTL.phys.data.PeterPaul, tn )
